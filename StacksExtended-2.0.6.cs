...
{
    [Info("StacksExtended", "k1lly0u", "2.0.6")]
    ...
    class StacksExtended : ChaosPlugin
    {
        ...
        private object CanStackItem(Item item, Item otherItem)
        {
            ...
            StackLimit otherStackLimit;
            m_StackLimits.Data.TryGetValue(item.info.shortname, out itemStackLimit);
            m_StackLimits.Data.TryGetValue(otherItem.info.shortname, out otherStackLimit);
           
            bool canStack = otherItem != item && (itemStackLimit?.GetStackSize() ?? item.info.stackable) > 1 && (otherStackLimit?.GetStackSize() ?? otherItem.info.stackable) > 1 && 
            ...
            if (Configuration.Options.BeltAntiToolWeaponStack && item.info == otherItem.info && 
                item.parent.HasFlag(ItemContainer.Flag.Belt) && item.GetHeldEntity() != null && 
                item.GetHeldEntity() is AttackEntity && !(item.GetHeldEntity() is ThrownWeapon) && !(otherItem.GetHeldEntity() is MedicalTool)) 
                return false;
        ...
        private object CanMoveItem(Item item, PlayerInventory playerInventory, uint targetContainerID, int targetSlot, int amount)
        {
            ...
            if (targetContainerID == 0U)
            {
                ...
                if (idealSlotEntity != null)
                    targetContainerID = idealSlotEntity.GetIdealContainer(playerInventory.baseEntity, item);
                
                if (targetContainerID == 0U && entityOwner is StorageContainer)
            ...
            if (item.parent != null)
            {
                ...
                if (targetSlot != -1 && itemContainer.entityOwner != item.parent.entityOwner)
                {
                    ...
                    {
                        int storageLimit = GetMaxStackable(otherItem, item.parent, false);
                        if (storageLimit > 0)
                        {
                            Debug.Log($"storagelimit {storageLimit}");
            ...
            amount = Mathf.Clamp(amount, 1, GetMaxStackable(item, itemContainer, true));
            
            if (playerInventory.baseEntity.GetActiveItem() == item)
                playerInventory.baseEntity.UpdateActiveItem(0U);
        ...
        private object OnMaxStackable(Item item)
        {
            ...
            return GetMaxStackable(item, item.parent, true);
        }

        //private System.Diagnostics.Stopwatch sw = new Stopwatch();
        private int GetMaxStackable(Item item, ItemContainer container, bool useItemFallback)
        {
            int maxStackable = 0;
                ...
                if (container.entityOwner)
                {
                    ...
                    if (container.entityOwner.OwnerID != 0UL)
                    {
                        foreach (KeyValuePair<string, VIPLimits> kvp in m_VIPLimits.Data)
                        {
                            ...
                            {
                                maxStackable = storageLimit.GetMaxStackable(item.info.shortname);
                                goto SKIP_BASIC;
                            }
                        }
                    }
            ...
            if (maxStackable == 0 && useItemFallback && m_StackLimits.Data.TryGetValue(item.info.shortname, out stackLimit))
                return stackLimit.GetStackSize();

            return maxStackable > 0 || !useItemFallback ? maxStackable : item.info.stackable;
        }
            ...
            foreach (ItemDefinition itemDefinition in ItemManager.itemList)
            {
                ...
                if (itemModDeployable)
                {
                    ...
                    if (storageContainer)
                    {
                        ...
                        if (!m_StorageLimits.Data.ContainsKey(storageContainer.PrefabName))
                        {
                            m_StorageLimits.Data[storageContainer.PrefabName] = new StorageLimit
                            {
                                MaxStackSize = storageContainer.maxStackSize,
                                ItemOverrides = new OrderedHash<string, StackLimit>(),
                                NiceName = PrefabNameToNiceName(storageContainer.PrefabName)
                            };
        ...
        private readonly string[] _replaceStrings = new string[]
        {
            ...
            "assets/prefabs/misc/chinesenewyear/sky_lantern/",
        ...
        private void OpenStacksUI(BasePlayer player)
        {
            ...
            ChaosUI.Destroy(player, STACKS_UI);
        ...
        private void CreateStorageEntry(UIUser uiUser, string t, BaseContainer layout, Anchor anchor, Offset offset)
        {
            ...
            ImageContainer.Create(layout, anchor, offset)
                ...
                .WithChildren(item =>
                {
                    ...
                    TextContainer.Create(item, Anchor.TopStretch, new Offset(74f, -35f, 0f, 0f))
                        .WithSize(12)
                        .WithText(storageLimit.NiceName)
                        .WithAlignment(TextAnchor.MiddleLeft);

                    TextContainer.Create(item, Anchor.HoriztonalCenterStretch, new Offset(74f, -15f, 0f, 5f))
                        .WithSize(12)
                    ...
                    ImageContainer.Create(item, Anchor.BottomStretch, new Offset(74f, 4.2f, -5f, 22.2f))
                        .WithStyle(m_ButtonStyle)
        ...
        private class ConfigData : BaseConfigData
        {
            ...
            public class ExcludeOptions
            {
                ...
                public bool IsExcluded(Item item) => item != null && (ExcludedSkins.Contains(item.skin) || ExcludedItems.Contains(item.info.shortname));
        ...
        public class StorageLimit
        {
            ...
            public int GetMaxStackable(string itemShortname)
            {
                StackLimit stackLimit;
                if (ItemOverrides.TryGetValue(itemShortname, out stackLimit))
                    return stackLimit.GetStackSize();
